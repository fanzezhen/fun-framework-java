...
编写新代码且涉及通用功能时读取，快速查找框架是否已提供对应功能。

## 脚手架信息

- GitHub: https://github.com/fanzezhen/fun-framework-java
- Gitee: https://gitee.com/fanzezhen/fun-framework-java
- 版本：动态（Spring Boot 4.x+）
- Java: 21+
- 查看最新版本：读取框架根目录 pom.xml
...
核心规则：
1. 先查后写：编写新功能前必须先查阅本文档
2. 深入文档：每个模块都有独立README.md，查阅以了解详细用法
3. 依赖继承：依赖版本由框架父POM统一管理，禁止覆盖版本号
4. 不确定时询问：优先询问用户或查阅框架文档，而非自行实现

开发流程：查阅本文档 → 找到对应模块 → 阅读README.md → 查看测试用例 → 编写代码
...
- **常用内容**:
  ...
  - 分页模型 `PageResult<T>` (包含分页元数据和数据列表)
  - **对象映射工具** `MapperFacadeUtil` (基于 Orika 实现 DTO/VO/Entity 转换)
  - 树形结构模型
  - 常用枚举和常量
...
**注意**: Controller 返回的数据会被 `ResponseBodyWrapper` **自动封装**为 `Result<T>` 格式,无需手动调用 `Result.success()`

**MapperFacadeUtil 使用示例**:
...
**使用 MapperFacadeUtil 前置条件**:
- **Spring Boot 项目**: 添加 `fun-framework-core-springboot` 依赖,自动注入 MapperFacade 实例
- **非 Spring 项目**: 可直接使用,内部会使用默认的 MapperFacade 实例
- **需要 Orika 依赖**: 添加 `ma.glasnost.orika:orika-core` 依赖
...
- **常用内容**:
  - 业务异常 ServiceException (完整类名: `com.github.fanzezhen.fun.framework.core.model.exception.ServiceException`)
...
**重要**: `ResponseBodyWrapper` 会自动将 Controller 返回的数据封装为:
```json
{
  "code": 200,
  "message": "success",
...
- **错误码**: 10***
- **功能**: Spring Boot 自动配置核心组件 (since v4.0.5)
- **文档**: [README.md](https://github.com/fanzezhen/fun-framework-java/blob/master/fun-framework-core/fun-framework-core-springboot/README.md)
- **常用内容**:
  - **MapperFacade 自动注入**: 自动将容器中的 `MapperFacade` Bean 注入到 `MapperFacadeUtil` 工具类
  - **零配置启动**: 添加依赖后自动生效,无需手动配置
  - **兼容非 Spring 环境**: `MapperFacadeUtil` 在非 Spring 环境下也可使用默认实例

**使用场景**:
- Spring Boot 项目使用 `MapperFacadeUtil` 进行对象映射
- 需要自定义 MapperFacade 配置(如字段映射规则、转换器)
- 确保 Orika 在 Spring 容器中正确初始化
...
```java
// 3. 直接使用 MapperFacadeUtil (无需手动配置)
@Service
public class UserService {
    
    public UserBO getUserById(Long id) {
        UserEntity entity = userDao.selectById(id);
        // 自动使用容器中的 MapperFacade
...
**注意事项**:
- **容器初始化时机**: MapperFacade 的注入发生在 `@PostConstruct` 阶段,确保在应用启动完成后才调用 `MapperFacadeUtil`
- **可选依赖**: Orika 依赖是可选的(`optional=true`),如果项目不需要对象映射功能,无需引入
- **线程安全**: `MapperFacadeUtil` 内部使用静态字段持有 MapperFacade 实例,线程安全由 Orika 保证
- **Java 9+ 兼容**: 使用 Orika 需要开放 `java.base` 模块的反射权限(参见模块 README)
...
public UserBO getUser(@PathVariable Long id) {
    UserBO user = userService.getById(id);
    if (user == null) {
        throw new ServiceException("用户不存在");  // GlobalExceptionHandler 会捕获并返回标准错误格式
    }
...
### 错误示例 4: 不使用框架提供的对象映射工具

```java
// 错误：手动编写 Entity → BO 转换代码
@Service
public class UserService {
    ...
    public PageResult<UserBO> pageUsers(Long pageNum, Long pageSize) {
        ...
        // 手动循环转换分页数据
        PageResult<UserBO> boPage = new PageResult<>();
        boPage.setCurrentPage(entityPage.getCurrentPage());
        boPage.setPageSize(entityPage.getPageSize());
        boPage.setTotal(entityPage.getTotal());
        
        List<UserBO> boList = new ArrayList<>();
        for (UserEntity entity : entityPage.getRowList()) {
            UserBO bo = new UserBO();
            bo.setId(entity.getId());
            bo.setName(entity.getUserName());
            // ... 更多字段映射
        ...
        boPage.setRowList(boList);
        return boPage;
    }
}

// 正确：使用框架提供的 MapperFacadeUtil
import com.github.fanzezhen.fun.framework.core.model.util.MapperFacadeUtil;
...
**注意**: 
- `MapperFacadeUtil` 基于 Orika 实现,性能优异(首次转换会生成字节码)
- 支持嵌套对象、集合、Map 等复杂类型的自动映射
- 同名字段自动映射,不同名字段可通过自定义 `MapperFacade` Bean 配置
- Spring Boot 项目添加 `fun-framework-core-springboot` 依赖后自动生效
...
| 需求       | 使用模块 | 关键类/注解 | 说明 |
|----------|---------|-----------|------|
| 统一返回格式   | core-web | ResponseBodyWrapper | **自动封装**,Controller 直接返回业务对象 |
| 分页查询     | core-model + data-mp | PageResult, IPage | - |
| 分页对象转换   | core-model + core-springboot | MapperFacadeUtil.page() | **推荐**: 自动转换分页数据中的行记录类型 |
| 对象映射(单个) | core-model + core-springboot | MapperFacadeUtil.map() | Entity/DTO/VO 互转 |
| 对象映射(集合) | core-model + core-springboot | MapperFacadeUtil.mapAsList() | 批量对象转换 |
| 业务异常     | core-exception | ServiceException | 直接抛出,GlobalExceptionHandler 统一处理 |
...
| 对象映射代理   | proxy-orika | OrikaMapper | 推荐使用 MapperFacadeUtil 替代 |
