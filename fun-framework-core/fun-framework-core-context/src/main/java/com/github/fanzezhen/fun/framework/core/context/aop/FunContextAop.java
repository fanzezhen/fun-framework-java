 ...
 * 上下文请求头校验和隐藏切面.
 * <p>
 * 配合 {@link ContextHeaderChecker} 注解使用，提供两种能力：
 * <ul>
...
public class FunContextAop {
     ...
     * 切点定义.
     * <p>
     * 拦截所有标注了 {@link ContextHeaderChecker} 注解的方法。
     */
    @Pointcut("@annotation(com.github.fanzezhen.fun.framework." +
            "core.context.aop.ContextHeaderChecker)")
    public void cut() {
    }
    ...
    public void doBefore(final JoinPoint joinPoint) {
        ContextHeaderChecker annotation = ((MethodSignature) joinPoint.getSignature())
                .getMethod()
                .getAnnotation(ContextHeaderChecker.class);
        if (annotation.requireds() != null) {
            for (String header : annotation.requireds()) {
                String context = ContextHolder.get(header);
                if (CharSequenceUtil.isEmpty(context)) {
                    throw new ServiceException(
                            FunContextExceptionEnum.CONTEXT_HEADER_MISSING,
                            header);
                }
            }
        }
    ...
    public Object doAround(final ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (annotation.hiddens() != null) {
            context = new JSONObject(annotation.hiddens().length);
            for (String header : annotation.hiddens()) {
