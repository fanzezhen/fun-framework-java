...
import com.github.fanzezhen.fun.framework.core.data.model.FileLineInfo;
...
public class StrFileUtil {
    ...
    public static <T> List<FileContentInfo<T>> collectFileData(String filePath,
                                                               List<LineMatchRule<T>> lineMatchRuleList,
        ...
        if (FileUtil.isDirectory(filePath)) {
            ...
            for (int i = 0; i < lines.size(); i++) {
                String line = lines.get(i);
                for (LineMatchRule<T> lineMatchRule : lineMatchRuleList) {
